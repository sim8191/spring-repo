...
// model에 딜어가는 데이터 중, Session에 보관시킬 데이터를 설정한는 주석
public class MemberController {
	
	@Autowired // 의존성주입
	private MemberService mService;// = new MemberServiceImpl(); //스프링에서는 직접 생성하면 스플링기능을 못씀
...
//	}
	...
	/*
	 * @RequestParam
	 *  - Servlet의 request.getParameter(키)를 대신 해주는 역활의 어노테이션
	 *  - 클라이언트가 요청한 값을 대신 변환하여 바인딩 해주는 역활은 arqumentResolver
	 *  가수행한다.
	 * */
//	public String login(
...
//		return "home";
//	}
	// 매개변수의 이름과 "일치"하는 request의 파라미터값을 추출하여 자동 바이딩.
...
//	}
	 ...
	 * */
	//@ModelAttribute Member m 이렇게 작성대도 됨
	/*public String login(Member m) { // 회원가입만들때 좋을거 같음
...
//		
//		// Jackson-databind를 활용하여, VO클래스, 컬랙션 데이터를 자동으로
//		// JSON으로 반환해줄 수 있다.
//		return searchMember;
//	}
	...
	public ResponseEntity<Member> selectOne(String userId){
		Member searchMemeber = mService.selectOne(userId);
		
		ResponseEntity<Member> res = null;
		if(searchMemeber != null) {
			// ok() : 응답상태 200
			res = ResponseEntity.ok(searchMemeber);
		}else { 
			// 응답상태 404
			res = ResponseEntity
					.notFound()
	 ...
	 *  - 컨트롤러에 예외처리 메서드 1개 추가한 후, @ExceptionHandler 어노테이션
	 *  추가
	 *  - 2순위로 적용
	 * 3. 어플리케이션 전역에서 발생하는 예외를 모아서 처리하는 방법
	 *  - 새로운 클래수 작성후 ControllerAdvice 어노테이션 추가
	 *  - 3순위
	 * */
