...
public class SecurityController {
	...
	public String accessDenied(Model m) {
		m.addAttribute("errorMsg","접근 불가능한 페이지 입니다.");
...
//		log.info("bcrypto : {}", passwordEncoder);
	 ...
	 *  - @ModelAttribut로 지정한 커맨드객체에 대해 바인딩 작업을 수행한다.
	 * 
	 * 처리순서
	...
	public void initBinder(WebDataBinder binder) {
		...
		dataFormat.setLenient(false); // yyMMdd형식이 아닌 경우 에러발생
		
		binder.registerCustomEditor(Date.class, "birthday",
				new CustomDateEditor(dataFormat, true));
	...
	public String register(
		...
		// 유효성 검사 성공시 비밀번호 암호화하여 회원 가입 진행
	...
	public String myPage(
		...
		// 2. SecurityuContextHolder를 이용
		Authentication auth2 = SecurityContextHolder.
		getContext().
		getAuthentication();
		// getAuthentication() == Authentication이거랑 같은거임
	...
	public String update(
		...
		if(result > 0) {
			...
			Authentication newAuth = 
					new UsernamePasswordAuthenticationToken(loginUser, auth.getCredentials(), 
							auth.getAuthorities());
