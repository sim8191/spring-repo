...
import com.kh.spring.board.modle.service.ReplyService;
 ...
 *  - 자원을 url이름을 구분하여 자원의 상태를 주고받는것
 *  
 *  @RestController
 *   - @ResponseBody + @Controller의 역활을 수행하는 컨트롤러
...
//		String userNo = ((MemberExt)auth.getPrincipal()).getUserName(); // 이름으로 할려면
		...
		return ResponseEntity.noContent().build();//성공201
	...
	public ResponseEntity<List<Reply>> selectList(int boardNo){
		...
		return ResponseEntity.ok(list); // 응답상태200
	}
